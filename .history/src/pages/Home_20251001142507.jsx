...
const handleAdd = () => {
  ...
  if (editIndex === -1) {
    ...
    axios.post("http://localhost:5000/lists", { action: trimmed })
      .then((res) => {
        setItems([...items, res.data.data]); // save full object from backend
        setInput("");
      })
    ...
    axios.put(`http://localhost:5000/lists/${itemToUpdate._id}`, { action: trimmed })
      .then((res) => {
        ...
        updatedItems[editIndex] = res.data.data; // replace with updated object
  ...
  axios.delete(`http://localhost:5000/lists/${_id}`)
