...
const Home = () => {
 ...
 axios.post("http://localhost:5000/lists", { action: input })
 ...
 const handleEdit = (index) => {
  const item = items[index];      // get the actual item object
  const id = item._id;            // get its _id

  // Send the updated value (from input) to backend
  axios.put(`http://localhost:5000/lists/${id}`, { action: input })
    .then((res) => console.log("Updated:", res.data))
    .catch((err) => console.log("Error:", err));

  // Update local state for editing
  setInput(item.action);           // show in input box
  setEditIndex(index);             // mark the index being edited
};
